...
const handleCompleteDeal = async () => {
    ...
    if (otherCompleted) {
      ...
      // Expire the flight and request
      await supabase.rpc('expire_old_flights');

      const { data: msgData } = await supabase.from('messages').insert([{
        match_id: activeMatch.id,
        sender_id: session.user.id,
        content: '✅ DEAL COMPLETED: Both parties have confirmed delivery. Escrow funds have been released to the traveler. Thank you for using Fetchr!',
        is_read: false
      }]).select();
  ...
  const agreeCancellation = async () => {
    ...
    const { data: msgData } = await supabase.from('messages').insert([{
      ...
      sender_id: session.user.id,
      content: hasEscrow
        ? '✅ CANCELLATION AGREED: This deal has been cancelled and the escrow payment will be refunded to the shipper within 5-10 business days.'
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on('postgres_changes', {
        ...
        // Update active match with latest data
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mx-6 my-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-800">
                  {getOtherParty(activeMatch)?.full_name || getOtherParty(activeMatch)?.email || 'User'}
                </p>
 ...
 {/* Complete Deal — both sides, only after flight date */}
              {(() => {
                const iAmTraveler = isTraveler(activeMatch);
                const myCompleted = iAmTraveler
                  ? activeMatch.traveler_completed
                  : activeMatch.shipper_completed;
                const otherCompleted = iAmTraveler
                  ? activeMatch.shipper_completed
                ...
                if (!flightHasPassed) {
                  return (
                    <div className="flex items-center gap-1.5 bg-gray-100 text-gray-400 px-3 py-2 rounded-xl text-xs font-semibold cursor-not-allowed"
                      title={`Available after ${new Date(flightDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}`}>
                      <Clock size={13} /> Complete (after {new Date(flightDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })})
                    </div>
                  );
                }
                ...
                return (
                  ...
                  >
                    {myCompleted ? (
                      ...
                      <><CheckCircle size={13} /> Confirm & Release</>
                    ) : (
          ...
          {/* Deal Summary Bar */}
          <div className="bg-purple-50 px-4 py-2 flex items-center gap-4 text-xs flex-wrap">
            ...
            <span className="flex items-center gap-1 text-purple-700">
              <Package size={12} />
              {activeMatch.request?.weight_kg}kg • {activeMatch.request?.item_name}
            </span>
            <span className="flex items-center gap-1 text-purple-700">
              <DollarSign size={12} />
          ...
          {getCompletionStatus(activeMatch) && (
            <div className={`px-4 py-2.5 flex items-center gap-2 text-sm font-medium border-b border-gray-100 ${
              getCompletionStatus(activeMatch)?.label.includes('Both')
                ? 'bg-green-50'
                : getCompletionStatus(activeMatch)?.label.includes('your turn')
                ? 'bg-purple-50'
                : 'bg-yellow-50'
            }`}>
            ...
            <div className="border-b border-gray-100 bg-red-50 p-4">
              ...
              <p className="text-xs text-red-600 mb-3">
                {activeMatch.status === 'in_escrow'
                  ? '⚠️ This deal has escrow funds. If both parties agree, the escrow will be refunded to the shipper.'
          ...
          {cancelRequest && cancelRequest.requested_by !== session.user.id && (
            <div className="border-b border-gray-100 bg-yellow-50 p-4">
              <p className="text-sm font-semibold text-yellow-700 mb-1 flex items-center gap-1">
                <AlertTriangle size={14} /> Cancellation Requested by Other Party
              </p>
              <p className="text-xs text-yellow-600 mb-3">
                Reason: "{cancelRequest.reason}"
                {activeMatch.status === 'in_escrow' && ' — If you agree, the escrow will be refunded to you.'}
              </p>
              <div className="flex gap-2">
                ...
                <button onClick={agreeCancellation} disabled={submittingCancel}
                  ...
                  {submittingCancel ? 'Processing...' : 'Agree & Cancel Deal'}
                </button>
              </div>
            </div>
          )}

          {/* Your own pending cancellation */}
          {cancelRequest && cancelRequest.requested_by === session.user.id && (
            <div className="border-b border-gray-100 bg-yellow-50 p-4">
            ...
            {messages.map((msg, i) => {
              ...
              if (isSystem) return (
                <div key={i} className="flex justify-center">
                  <div className="max-w-sm bg-gray-100 text-gray-600 text-xs px-4 py-2.5 rounded-xl text-center leading-relaxed">
                    {msg.content}
                  </div>
                </div>
              );
          ...
          <div className="p-4 border-t border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="w-11 h-11 bg-purple-600 rounded-xl flex items-center justify-center hover:bg-purple-700 transition disabled:opacity-50"
              >
