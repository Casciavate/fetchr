...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Which countries fetchr can actually pay out to via Stripe ──
    // Never a hardcoded list: Stripe restricts which countries a platform
    // may create connected accounts in, and getting it wrong fails hard
    // at account-creation time ("Connected accounts in CH cannot be
    ...
    const getSupportedPayoutCountries = async () => {
      ...
      try {
        const spec = await stripe.countrySpecs.retrieve(platformCountry)
    ...
    if (action === 'payout_countries') {
      const result = await getSupportedPayoutCountries()
    ...
    if (action === 'set_payout_country') {
      ...
      // Reject unsupported corridors here rather than letting
      // accounts.create fail later with Stripe's own raw error — by then
